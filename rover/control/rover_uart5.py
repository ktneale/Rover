...
Notes on IMU handling:
- The rover-board fused yaw is still forwarded as yaw_deg_raw for debugging.
- The published yaw_deg is now derived from integrated gyro-z (gz), because the
  board fused yaw has proven unreliable on the rover due to magnetic interference.
- Additional fields dtheta_deg / dtheta_rad are published for downstream SLAM.
"""
...
def first_present(msg: dict[str, Any], *keys: str) -> Any:
    for key in keys:
        if key in msg:
            return msg.get(key)
    return None
...
class RoverUartBridge:
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        # Integrated gyro heading state (derived from incoming IMU gz)
        self.imu_gyro_bias_samples: list[float] = []
        self.imu_gyro_bias_dps: float = 0.0
        self.imu_gyro_bias_ready: bool = False
        self.imu_gyro_filtered_dps: float = 0.0
        self.imu_heading_deg: float = 0.0
        self.last_imu_update_mono: Optional[float] = None
    ...
    def classify_uart_message(self, msg: dict[str, Any]) -> dict[str, Any]:
        ...
        if t == 1001:
            return {
                ...
                "p": msg.get("p"),
                "y": msg.get("y"),
                "temp_c": first_present(msg, "temp_c", "temp"),
                "v": first_present(msg, "v", "voltage"),
                "rssi": first_present(msg, "rssi", "wifi_rssi", "RSSI"),
                "raw": msg,
            }
    ...
    def telemetry_enabled(self) -> bool:
        return self.args.telemetry_pub_port > 0

    def publish_telemetry(self, obj: dict[str, Any]) -> None:
        if not self.telemetry_enabled():
            return
        udp_send_json(
            self.telemetry_sock,
            self.args.telemetry_pub_host,
            self.args.telemetry_pub_port,
            obj,
        )

    def build_telemetry_from_status(self, status_msg: dict[str, Any]) -> dict[str, Any]:
        return {
            "type": "telemetry",
            "ts": status_msg.get("ts", time.time()),
            "r": status_msg.get("r"),
            "p": status_msg.get("p"),
            "y": status_msg.get("y"),
            "voltage": status_msg.get("v"),
            "temp_c": status_msg.get("temp_c"),
            "rssi": status_msg.get("rssi"),
            "src": "rover_uart",
        }

    def maybe_update_gyro_bias(self, gz_raw_dps: float) -> None:
        """
        Estimate gyro bias from the first N near-stationary samples.
        Assumes rover is kept still on startup.
        """
        if self.imu_gyro_bias_ready:
            return

        if abs(gz_raw_dps) > self.args.imu_gyro_bias_max_abs_dps:
            return

        self.imu_gyro_bias_samples.append(gz_raw_dps)

        if len(self.imu_gyro_bias_samples) >= self.args.imu_gyro_bias_samples:
            self.imu_gyro_bias_dps = (
                sum(self.imu_gyro_bias_samples) / len(self.imu_gyro_bias_samples)
            )
            self.imu_gyro_bias_ready = True
            self.publish_event({
                "type": "imu_bias_ready",
                "ts": time.time(),
                "src": "rover_uart",
                "gyro_bias_dps": self.imu_gyro_bias_dps,
                "samples": len(self.imu_gyro_bias_samples),
            })

    def integrate_imu_gyro(
        self,
        imu_msg: dict[str, Any],
        ts_mono: float,
    ) -> dict[str, Any]:
        """
        Convert incoming IMU gyro-z into:
          - integrated heading_deg
          - dtheta_deg / dtheta_rad

        We keep the original board yaw as yaw_deg_raw for debugging, but
        publish yaw_deg as integrated gyro heading for compatibility with
        existing consumers.
        """
        out = dict(imu_msg)
        #return out

        gz_any = first_present(imu_msg, "gyro_z", "yaw_rate_dps")
        if gz_any is None:
            return out

        try:
            gz_raw_dps = float(gz_any)
        except Exception:
            return out

        self.maybe_update_gyro_bias(gz_raw_dps)

        bias = self.imu_gyro_bias_dps if self.imu_gyro_bias_ready else 0.0
        gz_corr_dps = gz_raw_dps - bias

        if abs(gz_corr_dps) < self.args.imu_gyro_deadband_dps:
            gz_corr_dps = 0.0

        alpha = clamp(self.args.imu_gyro_alpha, 0.0, 0.999)
        self.imu_gyro_filtered_dps = (
            alpha * self.imu_gyro_filtered_dps
            + (1.0 - alpha) * gz_corr_dps
        )

        if self.last_imu_update_mono is None:
            dt = 0.0
        else:
            dt = ts_mono - self.last_imu_update_mono
            if dt < 0.0:
                dt = 0.0
            if dt > self.args.imu_gyro_max_dt_s:
                dt = self.args.imu_gyro_max_dt_s

        self.last_imu_update_mono = ts_mono

        dtheta_deg = self.imu_gyro_filtered_dps * dt
        self.imu_heading_deg += dtheta_deg

        out["yaw_deg_raw"] = imu_msg.get("yaw_deg")
        out["yaw_deg"] = self.imu_heading_deg
        out["heading_deg"] = self.imu_heading_deg
        out["dtheta_deg"] = dtheta_deg
        out["dtheta_rad"] = math.radians(dtheta_deg)
        out["yaw_rate_dps_raw"] = gz_raw_dps
        out["yaw_rate_dps"] = self.imu_gyro_filtered_dps
        out["gyro_bias_dps"] = self.imu_gyro_bias_dps
        out["gyro_bias_ready"] = self.imu_gyro_bias_ready
        out["yaw_source"] = "integrated_gyro_z"
        return out
    ...
    def handle_uart_event(self, ev: UartEvent) -> None:
        ...
        if pub["type"] == "imu":
            pub = self.integrate_imu_gyro(pub, ev.ts)
            self.latest_imu = pub
        elif pub["type"] in ("board_status", "status"):
            self.latest_status = pub

        # Existing event stream stays unchanged
        self.publish_event(pub)

        # Additional local telemetry path for dashboard/Janus
        if pub["type"] == "status":
            self.publish_telemetry(self.build_telemetry_from_status(pub))
        elif pub["type"] == "imu":
            self.publish_telemetry({
                "type": "imu",
                "ts": pub.get("ts", time.time()),
                "yaw_deg": pub.get("yaw_deg"),
                "yaw_deg_raw": pub.get("yaw_deg_raw"),
                "heading_deg": pub.get("heading_deg"),
                "dtheta_deg": pub.get("dtheta_deg"),
                "dtheta_rad": pub.get("dtheta_rad"),
                "yaw_rate_dps": pub.get("yaw_rate_dps"),
                "yaw_rate_dps_raw": pub.get("yaw_rate_dps_raw"),
                "roll_deg": pub.get("roll_deg"),
                "pitch_deg": pub.get("pitch_deg"),
                "gyro_bias_dps": pub.get("gyro_bias_dps"),
                "gyro_bias_ready": pub.get("gyro_bias_ready"),
                "yaw_source": pub.get("yaw_source"),
                "src": "rover_uart",
            })
    ...
    def build_heartbeat_summary(self) -> dict[str, Any]:
        ...
        if self.latest_status is not None:
            ...
            if "rssi" in self.latest_status:
                summary["rssi"] = self.latest_status.get("rssi")

        if self.latest_imu is not None:
            if "yaw_source" in self.latest_imu:
                summary["yaw_source"] = self.latest_imu.get("yaw_source")
            if "yaw_deg" in self.latest_imu:
                summary["yaw_deg"] = self.latest_imu.get("yaw_deg")
            if "yaw_deg_raw" in self.latest_imu:
                summary["yaw_deg_raw"] = self.latest_imu.get("yaw_deg_raw")
            if "dtheta_deg" in self.latest_imu:
                summary["dtheta_deg"] = self.latest_imu.get("dtheta_deg")
            if "yaw_rate_dps" in self.latest_imu:
                summary["yaw_rate_dps"] = self.latest_imu.get("yaw_rate_dps")
            if "yaw_rate_dps_raw" in self.latest_imu:
                summary["yaw_rate_dps_raw"] = self.latest_imu.get("yaw_rate_dps_raw")
            ...
            if "gyro_bias_dps" in self.latest_imu:
                summary["gyro_bias_dps"] = self.latest_imu.get("gyro_bias_dps")
            if "gyro_bias_ready" in self.latest_imu:
                summary["gyro_bias_ready"] = self.latest_imu.get("gyro_bias_ready")
    ...
    def run(self) -> None:
        ...
        self.publish_event({
            ...
            "telemetry_pub_dest": (
                f"{self.args.telemetry_pub_host}:{self.args.telemetry_pub_port}"
                if self.telemetry_enabled()
                else "disabled"
            ),
            "imu_poll_s": self.args.imu_poll_s,
            "status_poll_s": self.args.status_poll_s,
            "imu_gyro_deadband_dps": self.args.imu_gyro_deadband_dps,
            "imu_gyro_alpha": self.args.imu_gyro_alpha,
            "imu_gyro_max_dt_s": self.args.imu_gyro_max_dt_s,
            "imu_gyro_bias_samples": self.args.imu_gyro_bias_samples,
            "imu_gyro_bias_max_abs_dps": self.args.imu_gyro_bias_max_abs_dps,
        })
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument(
        "--telemetry-pub-host",
        default="127.0.0.1",
        help="Optional telemetry publish destination host",
    )
    ap.add_argument(
        "--telemetry-pub-port",
        type=int,
        default=0,
        help="Optional telemetry publish destination port; <=0 disables",
    )
    ...
    ap.add_argument(
        "--imu-gyro-deadband-dps",
        type=float,
        default=1.0,
        help="Deadband applied to corrected gyro-z before integration",
    )
    ap.add_argument(
        "--imu-gyro-alpha",
        type=float,
        default=0.8,
        help="Low-pass filter coefficient for gyro-z; higher = smoother",
    )
    ap.add_argument(
        "--imu-gyro-max-dt-s",
        type=float,
        default=0.05,
        help="Maximum dt used for gyro integration to avoid spikes",
    )
    ap.add_argument(
        "--imu-gyro-bias-samples",
        type=int,
        default=40,
        help="Number of near-stationary startup samples to use for gyro bias estimation",
    )
    ap.add_argument(
        "--imu-gyro-bias-max-abs-dps",
        type=float,
        default=3.0,
        help="Only samples with abs(gz) below this are used for startup bias estimation",
    )
