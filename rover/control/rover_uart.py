...
- Read rover-board JSON messages from UART in a background threan
...
from rover.common.udp_utils import udp_send_json, udp_recv_json_nonblocking
...
class RoverUartBridge:
    ...
    def send_stop(self) -> None:
        # Adjust to match your rover’s actual stop command if needed
        self.send_uart_json({"T": 1, "L": 0, "R": 0})

    def old_send_drive_lr(self, left: int, right: int) -> None:
        left = int(clamp(left, -self.args.max_cmd, self.args.max_cmd))
        right = int(clamp(right, -self.args.max_cmd, self.args.max_cmd))
        # Adjust to match your board's actual motor command format
        self.send_uart_json({"T": 1, "L": left, "R": right})

    def old_send_cmd_vel(self, v: float, w: float) -> None:
        """
        Convert normalized robot motion command into left/right drive values.

        v: forward/backward request in [-1, +1]
        w: turn request in [-1, +1]
        """
        v = clamp(v, -1.0, 1.0)
        w = clamp(w, -1.0, 1.0)

        left = (v - w) * self.args.max_cmd
        right = (v + w) * self.args.max_cmd

        self.send_drive_lr(int(round(left)), int(round(right)))
    ...
    def request_imu(self) -> None:
        print("Sending on uart...")
        self.send_uart_json({"T": 126})

    def request_status(self) -> None:
        # Example placeholder; replace with your actual rover status/info request
    ...
    def classify_uart_message(self, msg: dict[str, Any]) -> dict[str, Any]:
        ...
        You will likely refine this once you know actual response payloads.
        """
        t = msg.get("T")

        # Example guesses based on your rover docs / commands shown so far
        if t == 126:
            return {
                "type": "imu",
                "ts": time.time(),
    ...
    def handle_uart_event(self, ev: UartEvent) -> None:
        ...
        if pub["type"] == "imu":
            self.latest_imu = ev.msg
        elif pub["type"] == "board_status":
            self.latest_status = ev.msg
    ...
    def build_heartbeat_summary(self) -> dict[str, Any]:
        ...
        if self.latest_status is not None:
            ...
            elif "temp" in self.latest_status:
                summary["temp_c"] = self.latest_status.get("temp")

        if self.latest_imu is not None:
            if "yaw_deg" in self.latest_imu:
                summary["yaw_deg"] = self.latest_imu.get("yaw_deg")
            elif "y" in self.latest_imu:
                summary["yaw_deg"] = self.latest_imu.get("y")
