...
class Controller:
    ...
    def publish_cmd_vel(self, v: float, w: float, source: str) -> None:
        ...
        # Only log if command actually changed
        changed = (
    ...
    def orer_publish_cmd_vel(self, v: float, w: float, source: str) -> None:
        self.stop_sent = False
    ...
    def handle_teleop_cmd(self, msg: dict[str, Any]) -> None:
        ...
        print(
            f"[TELEOP] v={self.last_teleop_cmd['v']:.3f} w={self.last_teleop_cmd['w']:.3f}",
            flush=True,
        )
    ...
    def apply_lidar_safety(self, v: float, w: float) -> tuple[float, float]:
        ...
        # reverse always allowed; also clear active states if backing away
        if v < 0.0:
        ...
        # hard stop if something is very close ahead
        ...
        # -------------------------------------------------
        # Steering bias away from closer front obstacle
        # -------------------------------------------------
        if v > 0.0 and not blocked_now:
            bias = 0.0
            nearest_front = min(front_left, front_right)

            if nearest_front < self.args.lidar_front_slow_mm:
                if front_right < front_left:
                    bias = +self.args.lidar_steer_bias   # steer left
                elif front_left < front_right:
                    bias = -self.args.lidar_steer_bias   # steer right

                if bias != 0.0:
                    reason_parts.append(f"bias={bias:+.2f}")
                    print(
                        f"[SAFETY] steering bias {bias:+.2f} "
                        f"(fl={front_left:.0f} fr={front_right:.0f})",
                        flush=True,
                    )
        ...
        # block steering into close side obstacles while moving ahead
        if v > 0.0:
            if left < self.args.lidar_side_stop_mm and w > 0.0:
                left_block_now = True
                w = 0.0
                reason_parts.append("left_turn_block")
                print(
                    f"[SAFE] left_turn_block triggered left={left:.0f} < {self.args.lidar_side_stop_mm:.0f}",
                    flush=True,
                )
            if right < self.args.lidar_side_stop_mm and w < 0.0:
                right_block_now = True
                w = 0.0
                reason_parts.append("right_turn_block")
                print(
                    f"[SAFE] right_turn_block triggered right={right:.0f} < {self.args.lidar_side_stop_mm:.0f}",
                    flush=True,
                )
    ...
    def old_apply_lidar_safety(self, v: float, w: float) -> tuple[float, float]:
        """
        Conservative first-pass LiDAR safety layer.
    ...
    def old_apply_lidar_safety(self, v: float, w: float) -> tuple[float, float]:
        ...
        # Fail-safe: stale LiDAR blocks forward motion
        if not self.lidar_is_fresh():
            if v > 0.0:
                v = 0.0
            return v, w
    ...
    def old_build_heartbeat_summary(self) -> dict[str, Any]:
        now = now_monotonic()
